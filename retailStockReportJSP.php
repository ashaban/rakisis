...
$op_query=mysql_query("select quantity,id,batch_number,buyingPrice,date from retailStock where itemId='$row[id]' and date<'$beg_date' and branch_id='$source_branch'");
while($op_row=mysql_fetch_array($op_query))
{
list($quantity_sold)=mysql_fetch_array(mysql_query("select sum(quantity) from sales where type='retail' and itemId='$row[id]' and batchNumber='$op_row[batch_number]' and date>='$op_row[date]' and date<'$beg_date' and branch_id='$source_branch'"));
$opening_balance=$op_row["quantity"]-$quantity_sold+$opening_balance;
$opening_cash=$op_row["buyingPrice"]*($op_row["quantity"]-$quantity_sold)+$opening_cash;
$quantity_sold=0;
}
//end of calculating opening balance

//Calculating Inward Quantity
list($quantity_inward)=mysql_fetch_array(mysql_query("select sum(quantity) from retailStock where itemId='$row[id]' and date between '$beg_date' and '$end_date' and branch_id='$source_branch'"));

//calculating Inward Cash
list($inward_cash)=mysql_fetch_array(mysql_query("select sum(buyingPrice*quantity) from retailStock where itemId='$row[id]' and date between '$beg_date' and '$end_date' and branch_id='$source_branch'"));

//calculating outward quantity
list($quantity_outward)=mysql_fetch_array(mysql_query("select sum(quantity_transfered) from items_transfer where itemId='$row[id]' and date_transfered between '$beg_date' and '$end_date' and source_branch='$source_branch' and transfer_type='retail'"));
...
while($row_out=mysql_fetch_array($out)) {
	list($bprice)=mysql_fetch_array(mysql_query("select buyingPrice from retailStock where id='$row_out[stock_id_from]'"));	
	$outward_cash=$outward_cash+($bprice*$row_out["quantity_transfered"]);
	}

//calculating Sales Quantity
list($quantity_sold)=mysql_fetch_array(mysql_query("select sum(quantity) from sales where itemId='$row[id]' and date between '$beg_date' and '$end_date' and type='retail' and branch_id='$source_branch' and batchNumber in(select batch_number from retailStock where  branch_id='$source_branch')"));

//calculating Sales Cash and total buying price of sold items
$sales_query=mysql_query("select batchNumber,quantity,(quantity*price) as cash from sales where itemId='$row[id]' and date between '$beg_date' and '$end_date' and type='retail' and branch_id='$source_branch' and batchNumber in(select batch_number from retailStock where branch_id='$source_branch')");
while($sales_row=mysql_fetch_array($sales_query))
{
$sales_cash=$sales_cash+$sales_row["cash"];
list($buying_price)=mysql_fetch_array(mysql_query("select buyingPrice from retailStock where batch_number='$sales_row[batchNumber]' and branch_id='$source_branch'"));
