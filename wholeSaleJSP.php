...
$results=mysql_query("select id from wholeStock where itemId='$_REQUEST[id]' and available>0 and branch_id='$branch_id' order by  expire_date DESC");
$x=1;
...
echo "</select></td></tr>";
echo "<tr style='background-color:#C0C0C0;'></td><td align='right'>Idadi Ya Piece</td><td align='left'><input type='text' name='quantity' style='width:90'></td></tr>";
...
list($sprice)=mysql_fetch_array(mysql_query("select selling_price from items where id='$_REQUEST[id]'"));
...
list($exp_date)=mysql_fetch_array(mysql_query("select expire_date from wholeStock where id='$values[batch]'"));
...
$branch_id=$_REQUEST["branch"];
$bgcolor='background-color:#A5E5E5;font-family:Geneva, Arial, Helvetica, sans-serif;';
list($id,$itemId,$quantity,$available,$price,$date_stocked,$exp_date)=mysql_fetch_array(mysql_query("select id,itemId,quantity,available,buying_price,date_stocked,expire_date from wholeStock where id='$_REQUEST[id]'"));
list($itemname,$unit,$total_units,$sale_price)=mysql_fetch_array(mysql_query("select name,unit,total_units,selling_price from items where id='$itemId'"));
...
echo "<center><tr><td colspan='3' align='center'><br><b>Maelezo Ya $itemname $unit $total_units Kwa Batch $id</b></td></tr>";
echo "<tr style='background-color:#515151;color:white;font-family:Georgia, 'Times New Roman', Times, serif;font-weight:bold;font-size:14;'><th>Jina La Bidhaa</th><th>Idadi Ya Piece Zilizopo</th><th>Bei Iliyonunuliwa Kwa Piece</th><th>Bei Yakuuzia Kwa Piece</th><th>Expire Date</th></tr>";
...
echo "<tr style='background-color:#515151;color:white;font-family:Georgia, 'Times New Roman', Times, serif;font-weight:bold;font-size:14;'><th>Jina La Bidhaa</th><th>Batch Number</th><th>Idadi Iliyopo</th><th>Bei Iliyonunuliwa Kwa Box</th><th>Tarehe Yakuingia Mzigo</th><th>Expire Date</th></tr>";
$results3=mysql_query("select * from wholeStock where itemId='$itemId' and available>0 and branch_id='$branch_id' order by expire_date DESC");
...
echo "<td align='center'>Batch $row[id]</td><td align='center'>$row[available]</td><td align='center'>".number_format($row["buying_price"])."</td><td align='center'>$date_stocked</td><td align='center'>$exp_date</td></tr>";
...
{
echo "<b><font color='red'>Error:Una Piece $quantities[$getBatch] Tuu Na Unataka Kuuza Piece $getQuantity,Mauzo Hayawezekani</font></b>";
}
else
{
...
list($stock_date)=mysql_fetch_array(mysql_query("select date_stocked from wholeStock where id='$batch'"));
...
else if($_REQUEST["from"]=="process_sale")
{
	...
	if(mysql_query("insert into sales (itemId,customer_id,quantity,price,date,salerId,batchNumber,type,pay_mode,branch_id,invoice_number) values ('$values[item]','$values[customer_id]','$values[quantity]','$values[sprice]','$values[date]','$_SESSION[id]','$values[batch]','whole','$pay_mode_save','$values[branch_id]','$values[invoice_number]')") or die(mysql_error())) {
		...
		mysql_query("update wholeStock set available='$available' where id='$values[batch]'");
	}
	}
