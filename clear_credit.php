...
xmlhttp.open("GET","clear_credit_JSP.php?name="+str+"&from=showNames",true);
xmlhttp.send();
}

function check(id)
{
document.getElementById("clr_cred_btn").disabled = true;
if (isNaN (document.getElementById(id).value-0))
{
document.getElementById("clr_cred_btn").disabled = false;
...
xmlhttp.open("GET","clear_credit_JSP.php?from=check&amount_paid="+document.getElementById(id).value+"&id="+id,true);
xmlhttp.send();
}
...
xmlhttp.onreadystatechange=function()
  ...
  if (xmlhttp.readyState==4 && xmlhttp.status==200)
    ...
    showCreditInfo(document.form.namesPool.value);    
    }
  }

xmlhttp.open("GET","clear_credit_JSP.php?from=submit&id="+id+"&amount_paid="+document.getElementById(id).value,true);
xmlhttp.send();
}

function showCreditInfo(customer_id)
{
if (window.XMLHttpRequest)
  {// code for IE7+, Firefox, Chrome, Opera, Safari
  xmlhttp=new XMLHttpRequest();
...
xmlhttp.open("GET","clear_credit_JSP.php?id="+customer_id+"&from=showCreditInfo",true);
xmlhttp.send();
}
...
document.getElementById("button").innerHTML="<input type='button' value='Load Debt Details' onclick='showCreditInfo(document.form.namesPool.value)'>";
}
</script>
...
<tr><td colspan="3" align="center"><h3>CLEAR CREDIT</h3></td></tr>
<tr style='background-color:#C0C0C0;'>
<td><b>Chagua Jina La Anaedaiwa</b></td><td><input type="text" name="names" onkeyup="showNames(this.value)" autocomplete="off" style="width:155"></td>
<td id="button"></td>
</tr>
<tr>
<td>&nbsp;</td>
