...
{
$debits=mysql_query("select id from debits where customer_id='$row[id]' and status='notCleared'");
if(mysql_num_rows($debits)==0)
continue;
...
{
$id=$_REQUEST["id"];
$results=mysql_query("select * from debits where customer_id='$id' and status='notCleared'");
echo "<div id='errmsg'></div>";
echo "<table><tr style='background-color:#515151;color:white;font-family:Georgia, Times New Roman, Times, serif;font-weight:bold;font-size:13;'><th>Jina</th><th>Kiasi Anachodai</th><th>Kiasi Kilicholipwa</th><th>Kiasi Kilichobaki</th><th>Tarehe Kiasi Kilikopwa</th><th>Maelezo Ya Ziada</th><th>Kiasi Kinacholipwa</th><th>Action</th></tr>";
...
echo "<tr style='background-color:#A5E5E5;font-family:Geneva, Arial, Helvetica, sans-serif'><td>$customer_name</td><td>".number_format($row["amount"])."</td><td>$cleared</td><td>$remainder</td></td><td>$date</td><td>$row[description]</td><td><input type='text' name='amount_cleared[$row[id]]' id='$row[id]'></td><td><input type='button' id='clr_deb_cred_btn' name='submit' value='Lipa' onclick='return check($row[id])'></td></tr>";
...
{
list($amount,$paid)=mysql_fetch_array(mysql_query("select amount,paid from debits where id='$_REQUEST[id]'"));
$debt=$amount-$paid;
...
{
mysql_query("update debits set paid='$paid',status='cleared' where id='$_REQUEST[id]'") or die(mysql_error());
echo "<b>Debit Cleared By $_REQUEST[amount_paid]</b>";
}
else 
{
mysql_query("update debits set paid='$paid' where id='$_REQUEST[id]'") or die(mysql_error());
